...
export function Skeleton({ className = "" }: SkeletonProps) {
  return (
    <div
      ...
      className={`animate-pulse bg-surface-container-high/60 ${className}`}
    />
  );
}
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" role="status" aria-label="Loading cards">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="p-5 glass-panel border border-glass-border shadow-sm space-y-3"
        >
...
export function TableSkeleton({ rows = 5, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="w-full glass-panel border border-glass-border overflow-hidden shadow-sm" role="status" aria-label="Loading table">
...
export function EmptyState({
  ...
  return (
    <div className="py-12 px-4 text-center flex flex-col items-center justify-center space-y-3 glass-panel border border-dashed border-glass-border">
      ...
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          className="atlas-btn atlas-btn-primary mt-2"
        >
          {actionLabel}
        </button>
      )}
...
export function ErrorState({
  ...
  return (
    <div
      role="alert"
      className="p-6 bg-rose-500/10 border border-rose-500/20 text-center space-y-3"
    >
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-rose-500/20 hover:bg-rose-500/30 active:scale-95 text-rose-300 border border-rose-500/30 text-xs font-semibold transition"
        >
